...
public class PlayerService implements PlayerRepository {
    ...
    @Override
    public Player getPlayersIds(int playerId) {
    ...
    public Player addPlayer(Player play) {
        play.setPlayerId(unique);
        team.put(unique, play);
        unique += 1;
        return play;
    }

    @Override
    public Player updatePlayer(int playerId, Player play) {
        Player existingPlayer = team.get(playerId);
        if (existingPlayer == null) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        }
        if (play.getplayerName() != null) {
            existingPlayer.setplayerName(play.getplayerName());
        }
        if (play.getjerseyNumber() >= 0) {
            existingPlayer.setjerseyNumber(play.getjerseyNumber());
        }
        if (play.getrole() != null) {
            existingPlayer.setrole(play.getrole());
        }
